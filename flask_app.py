...
@app.route('/pirm_uzd')
def uzd1():
   return '1. uzdevums atrisināts!'
@app.route('/otrs_uzd')
def uzd2():
   return str(random.randint(3, 9))
@app.route('/tresais_uzd/<int:n>')
def uzd3(n):
   return str(random.randint(n, n+10))
@app.route('/cet_uzd/<vards>')
def uzd4(vards):
    if vards[-1]=="s" or vards[-1] == "š" or vards[-1]=="is" or vards[-1]=="š" or vards[-1]=="š":
        vards=vards[:-1]
    return render_template("cet_template", uzd4=vards)
...
def index():
    ...
    if request.method == "GET":
        mycursor.execute("SELECT Viesi.Autors, Viesi.Komentars, Viesi.Datums, Vardi.Diena, Vardi.Menesis, MNOSAUKUMI.Menesis, MNOSAUKUMI.Nosaukums FROM Vardi RIGHT JOIN Viesi ON Vardi.Vards = Viesi.Autors LEFT JOIN MNOSAUKUMI ON Vardi.Menesis = MNOSAUKUMI.Menesis")
        rez=mycursor.fetchall()
        mydb.close()
        return render_template("main_page.html", comments=rez)
...
def rezultati():
    ...
    if request.method == "POST":
        dati = request.json
        if dati is not None and "vards" in dati and "punkti" in dati:
            resList.append(dati)
            with open(path, 'w') as f:
                json.dump(resList, f)
    sakartoti_rezultati = sorted(resList, key=lambda x: x.get("punkti", 0), reverse=True)
    return json.dumps(sakartoti_rezultati), 200, {'Content-Type': 'application/json'}

@app.route('/spele')
def spele():
     return render_template('kods.html')
@app.route('/cipari_test')
def cipari():
    resList=[]
    path='/home/lkupelis/mysite/templates/assets/Modelis.json'
    with open(path, 'r') as f:
        resList=json.load(f)
        return resList
...
@app.route('/modelis')
def modelis():
     return render_template('cipari.html')
@app.route('/list')
def list():
...
def login():
    ...
    values1 =(request.form["lietv"],)
    sql = "SELECT * FROM Paroles WHERE Username =%s"
    mycursor.execute(sql, values1)
    myresult = mycursor.fetchall()
    values2 = request.form["password"]
    if not myresult:
        return render_template("Login.html", error="Parole vai lietotājavards nav pareizi")
    user_record = myresult[0]
    parole = user_record[2]
    salt="q&%$jhgflkj )*"
    a_string=values2+salt
    hashed_string = hashlib.sha256(a_string.encode('utf-8')).hexdigest()
    mydb.close()
    if hashed_string == parole:
        session['loggedin'] = True
        session['username'] = request.form["lietv"]
        return redirect(url_for('index'))
    else:
        return render_template("Login.html", error="Parole vai lietotājavards nav pareizi")
...
def signup():
    ...
    dati1 = (request.form["lietv"],)
    if not (request.form["lietv"] and request.form["email"] and request.form["password"] and request.form["password1"] and request.form["password"] == request.form["password1"]):
        return render_template("SignUp.html", error="Nav aizpildīti visi lauki")
    else:
        mycursor = mydb.cursor()
        sql1="SELECT Username FROM Paroles WHERE Username =%s"
        mycursor.execute(sql1, dati1)
        myresult = mycursor.fetchall()
        if myresult:
            return render_template("SignUp.html", error="Tāds lietotājavārds jau ir")
        else:
            salt="q&%$jhgflkj )*"
            a_string=request.form["password"]+salt
            hashed_string = hashlib.sha256(a_string.encode('utf-8')).hexdigest()
            dati =(request.form["lietv"], request.form["email"], hashed_string)
            sql = "INSERT INTO Paroles (Username, Epasts, Parole) VALUES (%s,%s,%s)"
            mycursor.execute(sql, dati)
            mydb.commit()
            try:
                msg=Message("Labdien!", sender="leonidkp2007@gmail.com",recipients=[request.form["email"]])
                msg.body="Laipni lūdzu manā teritorijā!"
                mail.send(msg)
...
def logout():
    if session['loggedin']:
        ...
        return redirect(url_for('index'))
    return redirect(url_for('index'))
@app.route('/re_pass', methods=["GET", "POST"])
def re_pass():
    if request.method == "GET":
        return render_template("re_pass.html")
    if request.method == "POST":
        try:
            mydb = mysql.connector.connect(
                host="lkupelis.mysql.pythonanywhere-services.com",
            ...
            username = request.form["lietv"]
            print(username)
            dati1 = (username,)
            mycursor = mydb.cursor()
            sql1="SELECT Epasts FROM Paroles WHERE Username =%s"
            mycursor.execute(sql1, dati1)
            myresult = mycursor.fetchone()
            if not myresult:
                return render_template("re_pass.html", error="Tāda leitotājvārda neeksistē")
            tok=jwt.encode({'reset_password': username, 'exp':time() + 500}, key=app.secret_key,algorithm='HS256')
            msg= Message()
            msg.subject = "Password Reset"
            msg.sender = 'leonidskup2007@gmail.com'
            msg.recipients = [myresult[0]]
            msg.html = render_template('resetPwdEmail.html', token=tok, username=username)
            mail.send(msg)
            mydb.close()
            return render_template("re_pass.html", error="E-pasts aizsūtīts!")
        except Exception as e:
            return render_template("re_pass.html", error=f"Notika kļūda: {e}")
...
def resetPwd():
    ...
    if request.method == "GET":
        ...
        expTime = tok['exp']
        if time()<=expTime:
            return render_template("resetPwd.html", username=username)
        else: return "Tokena laiks ir beidzies"
    if request.method == "POST":
        username = request.form.get("username")
        passw = request.form.get("password")
        if not passw:
            return render_template("resetPwd.html", error="Ievadiet jaunu paroli")
        else:
            salt="q&%$jhgflkj )*"
            a_string=passw+salt
            hashed_string = hashlib.sha256(a_string.encode('utf-8')).hexdigest()
        try:
            ...
            mycursor = mydb.cursor()
            sql1 = "UPDATE Paroles SET Parole = %s WHERE Username = %s"
            mycursor.execute(sql1, (hashed_string, username))
            mydb.commit()
            mydb.close()
            return redirect(url_for('login'))
        except Exception as e:
            return render_template("resetPwd.html", error=f"Notika kļūda: {e}")
@app.route('/karte')
def karte():
...
@app.route('/aforisms')
def afor():
    return render_template('aforismi.html')